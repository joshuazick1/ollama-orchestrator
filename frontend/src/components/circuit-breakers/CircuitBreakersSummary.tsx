...
export const CircuitBreakersSummary = memo<CircuitBreakersSummaryProps>(({ breakers }) => {
  const openCount = breakers.filter(b => b.uiState === 'OPEN').length;
  const halfOpenCount = breakers.filter(b => b.uiState === 'HALF-OPEN').length;
  const closedCount = breakers.filter(b => b.uiState === 'CLOSED').length;
