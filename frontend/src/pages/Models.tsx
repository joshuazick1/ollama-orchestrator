...
import { getStatePriority } from '../utils/circuitBreaker';
...
const ServerBadge = ({
  ...
  // Compute worst state across all endpoints for this (server, model)
  const worstState =
    circuitBreakers.length > 0
      ? circuitBreakers.reduce(
          (worst, cb) =>
            getStatePriority(cb.uiState) < getStatePriority(worst) ? cb.uiState : worst,
          circuitBreakers[0].uiState
        )
      : undefined;
  const divergentCount = circuitBreakers.filter(b => b.uiState !== worstState).length;
  ...
  const currentScore = circuitBreakers[0]?.lbScore?.totalScore;
  const cachedScore = scoreCache.current.get(server.id);
  const displayScore = currentScore != null ? currentScore : cachedScore;

  // Determine state
  const isCircuitOpen = worstState === 'OPEN';
  const isCircuitHalfOpen = worstState === 'HALF-OPEN';
  const isTesting =
    isCircuitHalfOpen &&
    (circuitBreakers.find(b => b.uiState === 'HALF-OPEN')?.activeTestsInProgress || 0) > 0;
  ...
  return (
    ...
    >
      ...
      {divergentCount > 0 && (
        <span
          className="ml-1 px-1.5 py-0.5 bg-amber-500/20 text-amber-400 rounded text-[10px] font-medium"
          title={`${divergentCount} endpoint(s) with different state`}
        >
...
export const Models = () => {
  ...
  const { isLive } = useLiveUpdates({
    invalidateQueries: useMemo(
      () => [['modelMap'], ['servers'], ['all-models-status'], ['circuitBreakers'], ['in-flight']],
      []
    ),
  });
  ...
  const resetCbMutation = useMutation({
    ...
    onSuccess: (_data, variables) => {
      toastSuccess(`Reset all endpoints for ${variables.model} on ${variables.serverId}`);
  ...
  // Nested map: serverId -> model -> endpoint -> CircuitBreakerInfo
  const circuitBreakerByServerModelEndpoint = useMemo(() => {
    const map = new Map<string, Map<string, CircuitBreakerInfo>>();
    const breakers = circuitBreakersData?.circuitBreakers;
    safeArray<CircuitBreakerInfo>(breakers).forEach((cb: CircuitBreakerInfo) => {
      if (!cb.serverId || !cb.model) return;
      const key = `${cb.serverId}:${cb.model}`;
      if (!map.has(key)) map.set(key, new Map());
      if (cb.endpoint) map.get(key)!.set(cb.endpoint, cb);
    });
    return map;
  }, [circuitBreakersData]);
                            ...
                            <div key={server.id} className="flex items-center gap-1">
                              ...
                              <ServerBadge
                                ...
                                circuitBreakers={Array.from(
                                  circuitBreakerByServerModelEndpoint
                                    .get(`${server.id}:${model}`)
                                    ?.values() ?? []
                                )}
