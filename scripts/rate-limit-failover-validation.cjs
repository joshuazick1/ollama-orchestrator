...
const DURATION_MS = parseInt(
  process.argv.find(a => a.startsWith('--duration='))?.split('=')[1] || '60000'
);
...
async function fireRequest(id) {
  ...
  try {
    const response = await makeRequest(
      useModel,
      endpoint,
      useModel === EMBEDDINGS_MODEL ? embeddingsBody(useModel) : generateBody(useModel)
    );
...
async function checkClusterStatus() {
  try {
    const response = await new Promise((resolve, reject) => {
      http
        .get(`${ORCHESTRATOR_URL}/api/orchestrator/cluster-status`, res => {
          let data = '';
          res.on('data', chunk => (data += chunk));
          res.on('end', () => {
            try {
...
async function checkCircuitBreakers() {
  try {
    const response = await new Promise((resolve, reject) => {
      http
        .get(`${ORCHESTRATOR_URL}/api/orchestrator/circuit-breakers`, res => {
          let data = '';
          res.on('data', chunk => (data += chunk));
          res.on('end', () => {
            try {
...
async function runValidation() {
  ...
  try {
    const serversResp = await new Promise((resolve, reject) => {
      http
        .get(`${ORCHESTRATOR_URL}/api/orchestrator/servers`, res => {
          let data = '';
          res.on('data', chunk => (data += chunk));
  ...
  if (classificationResult.hasRateLimitedErrors) {
    ...
    if (classificationResult.sampleError) {
      console.log(
        `  Sample error: ${JSON.stringify(classificationResult.sampleError).substring(0, 200)}`
      );
    }
  } else {
  ...
  const progressInterval = setInterval(() => {
    ...
    process.stdout.write(
      `\r  Progress: ${stats.totalRequests} requests | ${stats.successfulRequests} OK | ${stats.rateLimitedRequests} 429s | ${stats.errorRequests} errors | Rate: ${rps} req/s   `
    );
  }, 1000);
  ...
  if (stats.uniqueServersRateLimited.size >= 2) {
    results.push(
      `✓ Multiple servers rate-limited (${stats.uniqueServersRateLimited.size} servers)`
    );
