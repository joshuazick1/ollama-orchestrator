...
describe('AIOrchestrator', () => {
  ...
  describe('getBestServerForModel', () => {
    ...
    it('should enforce server-level max concurrency across different models', () => {
      ...
      for (let i = 0; i < 2; i++) {
        orchestrator.incrementInFlight('server-1', 'model-b');
      }
    ...
    it('should allow routing when server total load is below max even if per-model is zero', () => {
      ...
      for (let i = 0; i < 3; i++) {
        orchestrator.incrementInFlight('server-1', 'model-x');
      }
  ...
  // DEPRECATED: manualTriggerRecoveryTest uses old CircuitBreaker system
  // RecoveryDriver handles probe-based recovery automatically
  // This test block needs to be rewritten to test RecoveryDriver behavior instead
  describe.skip('manualTriggerRecoveryTest', () => {
    beforeEach(() => {
      orchestrator.addServer(createServer({ id: 'server-1' }));
    });
  ...
  describe('Probe System - evictTuple', () => {
    ...
    it('should evict tuple and reset state to HEALTHY', async () => {
    ...
    it('should return undefined for non-existent tuple after eviction', async () => {
      const probeOrchestrator = orchestrator.getProbeOrchestrator();
      const tuple = { serverId: 'server-1', model: 'llama2', endpoint: 'ollama_chat' as const };

      await probeOrchestrator.evictTuple(tuple);
      expect(probeOrchestrator.getTupleState(tuple)).toBeUndefined();
    });
  });
  ...
  describe('Probe System - getAllStates', () => {
    ...
    it('should return all probe states as a Map', () => {
      const probeOrchestrator = orchestrator.getProbeOrchestrator();
      const allStates = probeOrchestrator.getAllStates();
      expect(allStates).toBeDefined();
      expect(allStates instanceof Map).toBe(true);
    });

    it('should return empty Map when no tuples exist', () => {
      const probeOrchestrator = orchestrator.getProbeOrchestrator();
      const allStates = probeOrchestrator.getAllStates();
      expect(allStates.size).toBe(0);
    });

    it('should include tuple states after recording probe results', async () => {
      const probeOrchestrator = orchestrator.getProbeOrchestrator();
      const tuple = { serverId: 'server-1', model: 'llama2', endpoint: 'ollama_chat' as const };
      ...
      expect(probeOrchestrator.getTupleState(tuple)).toBeDefined();
    });
  });
  ...
  describe('Probe System - canServe for routing', () => {
    beforeEach(() => {
      orchestrator.addServer(createServer({ id: 'server-1' }));
      const s1 = orchestrator.getServer('server-1');
      if (s1) {
    ...
    it('should block routing when probe state is UNHEALTHY', () => {
      const probeOrchestrator = orchestrator.getProbeOrchestrator();
      const tuple = { serverId: 'server-1', model: 'llama2', endpoint: 'ollama_chat' as const };
    ...
    it('should allow routing when probe state is HEALTHY', () => {
      const probeOrchestrator = orchestrator.getProbeOrchestrator();
      const tuple = { serverId: 'server-1', model: 'llama2', endpoint: 'ollama_chat' as const };
    ...
    it('should allow routing when probe state is SUSPECT', () => {
      const probeOrchestrator = orchestrator.getProbeOrchestrator();
      const tuple = { serverId: 'server-1', model: 'llama2', endpoint: 'ollama_chat' as const };
    ...
    it('should block routing when probe state is RECOVERING', () => {
      const probeOrchestrator = orchestrator.getProbeOrchestrator();
      const tuple = { serverId: 'server-1', model: 'llama2', endpoint: 'ollama_chat' as const };
  ...
  describe('Probe System - evictTuple for model', () => {
    beforeEach(() => {
      orchestrator.addServer(createServer({ id: 'server-1' }));
    });

    it('should evict tuple and subsequent eviction returns no state', async () => {
      const probeOrchestrator = orchestrator.getProbeOrchestrator();
      const tuple = { serverId: 'server-1', model: 'llama2', endpoint: 'ollama_chat' as const };
